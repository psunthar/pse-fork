        ...
        if zero == 'now':
            ...
            if zero < 0:
                hoomd.context.msg.error(
                    "Cannot create a shear_function variant with a negative zero\n"
                )
                raise RuntimeError('Error creating shear function')
            if zero > hoomd.context.current.system.getCurrentTimeStep():
                hoomd.context.msg.error(
                    "Cannot create a shear_function variant with a zero in the future\n"
                )
                raise RuntimeError('Error creating shear function')
            self._offset = zero
...
class sine(_shear_function):
    ...
    def __init__(self, dt, shear_rate, shear_freq, zero='now'):
        ...
        if shear_rate <= 0:
            hoomd.context.msg.error(
                "Shear rate must be positive (use steady class instead for zero shear)\n"
            )
            raise RuntimeError("Error creating shear function")
        if shear_freq <= 0:
            hoomd.context.msg.error(
                "Shear frequency must be positive (use steady class instead for steady shear)\n"
            )
            raise RuntimeError("Error creating shear function")
